...
Apify.main(async () => {
  ...
  let countryUrl;
  countryUrl =
    countryDict[country.toLowerCase()] ||
  ...
  // IF NO START URL => CREATING FIRST "LIST"  PAGE ON OUR OWN
    ...
    log.info(
      `Running site crawl country ${country}, position ${position}, location ${location}`
    );
    const startUrl = `${countryUrl}/jobs?${
      position ? `q=${encodeURIComponent(position)}&sort=date` : ""
    ...
    handlePageFunction: async ({ $, request, session, response }) => {
      ...
      switch (request.userData.label) {
        ...
        case "LIST":
          ...
          currentPageNumber++;
          const hasNextPage =
            $(`a[aria-label="${currentPageNumber}"]`).length > 0 ? true : false;
        ...
        case "DETAIL":
          if (!(maxItems && itemsCounter > maxItems)) {
            let result = {
              ...
              salary:
                $("#salaryInfoAndJobType .attribute_snippet").text() !== ""
                  ? $("#salaryInfoAndJobType .attribute_snippet").text()
                  : null,
              ...
              rating: $('meta[itemprop="ratingValue"]').attr("content")
                ? Number($('meta[itemprop="ratingValue"]').attr("content"))
                : null,
              reviewsCount: $('meta[itemprop="ratingCount"]').attr("content")
                ? Number($('meta[itemprop="ratingCount"]').attr("content"))
                : null,
              url: request.url,
              id: getIdFromUrl(
